...
(function() {
  var DEFAULT_OPTIONS, HtmlParser, Node, PREVIOUS_MD, REGEX, REPLACEMENTS, R_IGNORE_CHILDREN, R_PARAGRAPH_ONLY, key, md, result, value, _ref, _ref1, _ref2,
  ...
  Node = (_ref = window.Node) != null ? _ref : {};
  ...
  HtmlParser = (function() {
    ...
    function HtmlParser(html, options) {
      ...
      this.buffer = '';
      this.exceptions = this.links = this.linkTitles = [];
      this.inCode = this.inPre = this.inOrderedList = this.parsed = false;
      this.last = null;
      this.left = '\n';
      this.linkCache = this.unhandled = {};
    ...
    HtmlParser.prototype.parse = function() {
      var container, i, link, tag, unhandledTags, _fn, _i, _len, _ref3,
        _this = this;
      if (!this.html) {
        return '';
      }
      ...
      _fn = function(link, i) {
        var title;
        title = _this.linkTitles[i] ? " \"" + _this.linkTitles[i] + "\"\n" : '\n';
        if (link) {
          return _this.append("[" + i + "]: " + link + title);
        }
      };
      ...
      if (this.options.debug) {
        ...
        if (this.exceptions.length) {
          console.log(this.exceptions.join('\n'));
        }
      }
      this.append('');
      this.parsed = true;
  ...
  md = window.md = function(html, options) {
