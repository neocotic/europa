(function() {
  var NOT_FOUND, OptionParser, R_HTML_EXT, SWITCHES, exit, extension, fs, glob, md, opts, outputPath, parseHtml, parseOptions, parsePath, path, sources, writeMarkdown;
  ...
  parseOptions = function() {
    var parser, pattern, toCamelCase;
    ...
    pattern = parser.parse(process.argv).slice(2).join(' ');
    if (pattern) {
      return sources = glob(pattern, {
        nosort: true
      });
    } else {
      return exit(parser.toString());
